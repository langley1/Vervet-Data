...
#This plot shows that there are the most "Human Foods" in the high and medium human level locations and spilled enclosure food only in the low level group
```
...
## Finding the proportion of the food desc categories in **high human** category

*Total*
```{r}
nrow(fulljoinDF_noNA) #With the NAs removed from the food descriptor column, there are a total of 8252 

allfoods_high_human<- fulljoinDF_noNA %>% filter(HumanLevel == "H")
...
```{r}
myplot4 <- ggplot(humanlevel_ALL, aes(HumanLevel, group = Age)) + 
          geom_bar(aes(y = ..prop.., fill = factor(..x..)), stat="count") + 
          scale_y_continuous(labels=scales::percent) +
          ylab("relative frequencies") +
          facet_grid(~Age)
